...
class ModbusCLIApp:
    ...
    def __init__(
        ...
        session_cls: Type[ModbusSession] = ModbusSession,
    ) -> None:
        ...
        while True:
            action = (
                self.prompt_service.prompt(
                    "Pilih aksi - [r]ead, [w]rite, [q]uit", cast=str
                )
                .strip()
                .lower()
            )
    ...
    def _handle_read(self) -> None:
        ...
        count = self.prompt_service.prompt(
            "Jumlah register yang dibaca", default=1, cast=int
        )
        try:
            ...
            if response.isError():
                ...
                print(
                    f"✅ Data dari alamat {address} s/d {address + count - 1}: {response.registers}"
                )
    ...
    def _handle_write(self) -> None:
        assert self.session is not None
        address = self.prompt_service.prompt("Alamat register (holding)", cast=int)
        value = self.prompt_service.prompt(
            "Nilai yang akan ditulis (0-65535)", cast=int
        )
        try:
